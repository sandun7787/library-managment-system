...
if(isset($_POST['query'])) {
    if($_POST['query'] == null) {
        ...
        $conn = getCon();
        $query = "SELECT `id`, `book_id`, `borrower_id`, `borrow_date`, `due_date`, `return_date`, `penalty` FROM `records` WHERE  penalty IS NOT NULL AND NOW() > due_date ORDER BY due_date ASC ";
        ...
        include '../connection/config.php';
        $conn = getCon();
        $query = "SELECT `id`, `book_id`, `borrower_id`, `borrow_date`, `due_date`, `return_date`, `penalty` FROM `records` WHERE  penalty IS NOT NULL AND NOW() > due_date AND borrower_id LIKE '{$_POST['query']}%' LIMIT 100";
        $result = $conn->query($query);
        $count = $result->rowCount();
